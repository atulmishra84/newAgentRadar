...
If you are deploying to your domain, you **must use HTTPS**. The default `nginx.conf` provided in the repository is already perfectly configured for this.
...
3. Ensure your `nginx.conf` references them correctly (the default config expects `/etc/nginx/certs/cert.pem` and `/etc/nginx/certs/key.pem`).
4. Copy the `certs/` folder to your Azure VM along with the rest of your files.
