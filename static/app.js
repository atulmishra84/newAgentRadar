...
function doLogin() { return (async () => {
  ...
  try {
    if (_apiToken) {
      const p = JSON.parse(atob(_apiToken.split('.')[1]));
      if (p.name) displayName = p.name;
      if (p.role) currentRole = p.role || 'viewer';
    }
  } catch(e) {}
...
function applyRBAC() {
  ...
  function hideBtn(el) { if (el) el.style.display = 'none'; }
  function lockSelector(sel, reason) {
    document.querySelectorAll(sel).forEach(b => lockBtn(b, reason));
  }
  ...
  if (isViewer) {
    ...
    lockSelector('[onclick*="openModal('modal-webhooks"]', 'Viewer cannot configure webhooks');
    lockSelector('[onclick*="openModal('modal-evidence"]',  'Viewer cannot export evidence');
